...
def define_joint_model(result_dict, parameters, null_dataset = [True, False]):
    ...
    for submodels, sub_state_dict in zip(full_model.models, list_state_dict):
        submodels.load_state_dict(sub_state_dict, strict=False)
    ...
    # Loop through model list and join models (control and perturbation) for each protein 
    for i in range(0, len(full_model.models), 2):
        ...
        combined_model = ExactGPModel(combined_train_input, combined_train_target, combined_likelihood, lengthscale_prior=parameters['lengthscale_prior'], lengthscale_minconstraint=parameters['lengthscale_minconstraint'], mean=gpytorch.means.ZeroMean(), lengthscale_mult=parameters['lengthscale_mult'])
        ...
        if null_dataset == False:
            ...
            # prior_joint_df will be created inside the sampling loop for multiple samples
            ...
            for sample_idx in range(samples_per_id):
                ...
                prior_joint_df_sample.loc[:, 'uniqueID'] = np.repeat(f'{protein_id}_sample_{sample_idx + 1}', len(temp_flat))
                prior_joint_df_sample.loc[:, 'x'] = temp_flat
    ...
    if null_dataset == False:
        ...
        join_model_result_dict.update({
                ...
                "sampled_prior_df": pd.concat(all_samples)  # Ensure all samples are concatenated
                })
    ...
    def __init__(self, train_x, train_y, likelihood, mean = gpytorch.means.ZeroMean(), lengthscale_prior=None, lengthscale_minconstraint = ['min', 'mean', 'median', 'max', None],lengthscale_mult: float = 1.0):
        ...
        if not lengthscale_minconstraint is None:
            ...
            if lengthscale_minconstraint == 'min':
                Constt = lengthscale_mult*min(DistVec) # we use the minimum distance between temperatures as a lower limit for the lengthscale
